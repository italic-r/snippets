...
-- Hex mode filetypes
vim.g.hexmode_patterns = { "*.hex", "*.bin", "*.exe", "*.dat", "*.o" }
...
-- Highlighting bad whitespace determined per filetype
vim.cmd([[ hi BadWhitespace ctermbg=darkred guibg=darkred]])
...
-- Reload file and restore selection
vim.keymap.set("n", "<F5>", ":e!<CR>")
vim.keymap.set("v", "<F5>", "<Esc><F5>gv")
vim.keymap.set("i", "<F5>", "<C-O><F5>")

-- Save buffer and restore selection
vim.keymap.set("n", "<F8>", ":update<CR>")
vim.keymap.set("v", "<F8>", "<Esc><F8>gv")
vim.keymap.set("i", "<F8>", "<C-O><F8>")

-- Copy/paste with system clipboard
vim.keymap.set("v", "<Leader>c", '"+y')
vim.keymap.set("v", "<Leader>vv", '"+p')
...
function StripTrailingWhitespaceKey()
	--[[
    rust: nnoremap <F4> :let _s=@/<Bar>:%s`\v^(\s*//[^/!]{-}\|\s*[^/]{-}\|\n)(\s+)$`\1`ge<Bar>:let @/=_s<Bar><CR>
    else: nnoremap <F4> :let _s=@/<Bar>:%s`\v\s+$``ge<Bar>:let @/=_s<Bar><CR>
  --]]
end
...
require("lazy").setup({
	...
	{ -- Automatically insert delimiter pairs
...

	{
		...
		ft = { "markdown" },
		keys = { "<Leader>mp", ":MarkdownPreview<CR>" },
	},
	{
